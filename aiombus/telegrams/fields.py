...
class AddressField(TelegramField):
    ...
    def is_configured_slave(self) -> bool:
        return (
            AF_SLAVE_MIN_RANGE_VALUE_BYTE <= self.byte <= AF_SLAVE_MAX_RANGE_VALUE_BYTE
        )

    def is_slave(self) -> bool:
        return AF_UNCONFIGURED_SLAVE_BYTE <= self.byte <= AF_SLAVE_MAX_RANGE_VALUE_BYTE
...
class ControlField(TelegramField):
    ...
    If 1, than it is interpreted as a calling direction, else, a reply direction.
    ...
    The DFC serves to control the flow of data, in that the slave with a DFC=1
    ...
    Such Class 1 data is of higher priority, which (in contrast to Class 2 data) should
    be transmitted as soon as possible.
    The support of Class 1 data and the bits DFC and ADC is not required by the standard.

    The bits 0 to 3 of the control field code the true function or action of the message.
    """
...
### Value Information Fields section
